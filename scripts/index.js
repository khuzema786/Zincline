...
const imageCompressExclude = ["slider-1.png", "slider-2.jpeg"]
...
const main = async () => {
  ...
  for (let i = 0; i < htmlFiles.length; i++) {
    ...
    if (htmlFiles[i].match(/thankyou\.html$/gm)) {
      data = template
        .toString()
        .replace("{{header}}", "")
        .replace("{{footer}}", "")
  ...
  await exec(
    `find "${dirname}/dist" -type d -print0 | xargs -0 chmod 755 && 
    find "${dirname}/dist" -type f -print0 | xargs -0 chmod 644`
  );

  if (!isDevelopment && process.argv[2] !== "--no-image-compress") {
    await exec("rm -rf dist/assets/images/*");
    ...
    await exec("rm -rf log", { recursive: true, force: true });

    imageCompressExclude.map(image => {
      exec(`rm -rf "${dirname}/dist/assets/images/${image}" && cp -r "${dirname}/src/assets/images/${image}" "${dirname}/dist/assets/images/${image.split("/").slice(0, image.split("/").length - 1).join("/")}"`)
    })
  }
};
